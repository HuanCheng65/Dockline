...
//
// 1327 / 1328 一并挂上，只为在时间线上标出转场的起止。**实测这两个号在 Space 切换时
// 一次都不发**：抓到了整段位移连续变化的时间线，中间没有任何一条事件行。它们是调度中心
// 那条线索上的号（见 MissionControl.swift），与 Space 转场无关。Space 转场的事件号还没定，
// 要找就用现成的 `docklinespike events 1200 1600 60`，一边滑一边看谁响。
...
func commandSpacesWatch(seconds: Double, hz: Double, extraWIDs: [CGWindowID]) {
    ...
    print("""
      ...
      （1327 / 1328 实测在 Space 切换时不发，时间线上没有事件行是正常的。）
